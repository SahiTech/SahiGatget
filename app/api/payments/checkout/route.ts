...
export async function POST(request: Request) {
  try {
    ...
    if (String(order.payment_requirement) === 'COD' || String(order.payment_method).toUpperCase() === 'COD') return NextResponse.json({ ok: false, message: 'Cash on Delivery does not require online payment.' }, { status: 409, headers: { 'Cache-Control': 'private, no-store' } })
    ...
    const message = error instanceof Error && error.message === 'PAYMENT_PROVIDER_NOT_CONFIGURED' ? 'Online payment is not available right now. Please choose Cash on Delivery or contact support.' : 'We could not start the secure payment session. Please try again.'
