...
function QuantitySelector({ section, quantity, onChange }: { section: Extract<LandingSection, { type: 'quantity_selector' }>; quantity: number; onChange: (value: number) => void }) {
  ...
  return <section className={`mx-auto max-w-5xl px-4 py-5 sm:px-6 ${visibility(section)}`}><div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100"><div><h2 className="text-lg font-black text-slate-950">{section.title || 'পরিমাণ নির্বাচন করুন'}</h2><p className="mt-1 text-xs text-slate-500">আপনার পছন্দ অনুযায়ী পরিমাণ নির্ধারণ করুন।</p></div><div className="inline-flex items-center rounded-xl border border-slate-200 bg-white p-1"><button type="button" onClick={() => onChange(Math.max(1, quantity - 1))} aria-label="পরিমাণ কমান" className="h-10 w-10 rounded-lg text-lg font-black text-slate-700 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500">−</button><span aria-live="polite" className="min-w-10 text-center text-sm font-black text-slate-950">{quantity}</span><button type="button" onClick={() => onChange(Math.min(max, quantity + 1))} aria-label="পরিমাণ বাড়ান" className="h-10 w-10 rounded-lg text-lg font-black text-slate-700 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500">+</button></div></div></section>
}
