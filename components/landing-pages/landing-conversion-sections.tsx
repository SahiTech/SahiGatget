...
function imageForVariant(product: StorefrontProduct, variant?: StorefrontVariant) {
...
const initialForm: FormState = { fullName: '', phone: '', email: '', division: '', district: '', area: '', address: '', postalCode: '', notes: '' }

function scrollToOrder() {
  document.getElementById('order-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
}

function ProductHero({ section, product, variant }: { section?: Extract<LandingSection, { type: 'hero' }>; product?: StorefrontProduct; variant?: StorefrontVariant }) {
...
function QuantitySelector({ section, quantity, onChange }: { section: Extract<LandingSection, { type: 'quantity_selector' }>; quantity: number; onChange: (value: number) => void }) {
  ...
  return <section className={`mx-auto max-w-5xl px-4 py-5 sm:px-6 ${visibility(section)}`}><div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100"><div><h2 className="text-lg font-black text-slate-950">{section.title || 'পরিমাণ নির্বাচন করুন'}</h2><p className="mt-1 text-xs text-slate-500">চূড়ান্ত স্টক সার্ভারে যাচাই হবে।</p></div><div className="inline-flex items-center rounded-xl border border-slate-200 bg-white p-1"><button type="button" onClick={() => onChange(Math.max(1, quantity - 1))} aria-label="পরিমাণ কমান" className="h-10 w-10 rounded-lg text-lg font-black text-slate-700 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500">−</button><span aria-live="polite" className="min-w-10 text-center text-sm font-black text-slate-950">{quantity}</span><button type="button" onClick={() => onChange(Math.min(max, quantity + 1))} aria-label="পরিমাণ বাড়ান" className="h-10 w-10 rounded-lg text-lg font-black text-slate-700 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500">+</button></div></div></section>
}
...
function Countdown({ section }: { section: Extract<LandingSection, { type: 'countdown' }> }) {
  ...
  return <section role="timer" aria-live="polite" className={`mx-auto max-w-5xl px-4 py-4 sm:px-6 ${visibility(section)}`}><div className="rounded-2xl bg-slate-950 p-5 text-white"><p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-300">ক্যাম্পেইন সময়</p><h2 className="mt-2 text-xl font-black">{section.title}</h2>{section.body ? <p className="mt-2 text-sm leading-6 text-slate-300">{section.body}</p> : null}<p className="mt-4 text-xl font-black text-emerald-300">{remaining || 'হিসাব হচ্ছে…'}</p></div></section>
}
...
export function LandingConversionSections({ sections, products }: { sections: LandingSection[]; products: StorefrontProduct[] }) {
  ...
  return <div className="min-h-screen overflow-x-clip bg-white pb-24 text-slate-950"><ProductHero section={hero} product={selectedProduct} variant={selectedVariant} />
    {sections.map((section) => {
      if (section.enabled === false || section.type === 'hero' || section.type === 'product' || section.type === 'announcement' || section.type === 'product_gallery' || section.type === 'related_products' || section.type === 'sticky_mobile_cta') return null
      ...
      if (section.type === 'delivery_info' || section.type === 'warranty' || section.type === 'trust' || section.type === 'social_proof') return <TextSection key={section.id} section={section} tone={section.type === 'delivery_info' ? 'green' : 'plain'} title={section.type === 'delivery_info' ? 'ডেলিভারি ও পেমেন্ট' : section.type === 'social_proof' ? 'গ্রাহক রিভিউ' : undefined} />
