...
export function TrustStrip() {
  return (
    <section aria-label="Store benefits" className="border-b border-slate-200 bg-white">
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-2 px-3 py-3 sm:gap-3 sm:px-6 sm:py-5 lg:grid-cols-4 lg:px-8">
        {items.map(({ icon: Icon, title, detail }) => (
          <div key={title} className="flex min-w-0 items-center gap-2.5 rounded-xl border border-slate-100 bg-slate-50/80 px-2.5 py-2.5 sm:gap-3 sm:rounded-2xl sm:p-4">
            ...
            <div className="min-w-0">
              <p className="truncate text-[11px] font-black text-slate-950 sm:text-sm">{title}</p>
