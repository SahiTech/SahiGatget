  ...
  return <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start">
    <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      {result && <div className="mt-8 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.15em] text-emerald-700">Order found</p><p className="mt-2 font-mono text-sm font-black text-slate-950">{result.orderNumber}</p><p className="mt-1 text-xs text-slate-500">Placed {dateLabel(result.createdAt)}</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-emerald-800">{statusLabel(result.status)}</span></div>
