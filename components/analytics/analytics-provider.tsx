...
export function AnalyticsProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const sync = () => setConsent(getAnalyticsConsent()); window.addEventListener('sahigadget-consent-change', sync); sync(); trackPageView(); return () => window.removeEventListener('sahigadget-consent-change', sync) }, [])
