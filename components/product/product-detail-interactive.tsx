...
import { Check, CheckCircle2, ChevronLeft, ChevronRight, Loader2, MapPin, PackageCheck, Phone, ShoppingCart, ShieldCheck } from 'lucide-react'
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,.92fr)_minmax(0,1.08fr)] lg:gap-16">
      ...
      <div className="min-w-0">
        ...
        <div className="mt-6 rounded-[1.5rem] border border-emerald-200 bg-emerald-50 p-5"><p className="font-black text-slate-950">Ready to order?</p><p className="mt-1 text-sm leading-6 text-slate-600">Cash on Delivery is available. Final delivery charge and stock are checked through the existing order flow.</p>{selected && selected.is_in_stock ? <div className="mt-4 flex flex-wrap gap-3"><button type="button" disabled={cartBusy} onClick={addSelectedToCart} className="group inline-flex min-h-12 items-center justify-center gap-2 rounded-full border-2 border-slate-950 bg-slate-950 px-6 py-3 text-sm font-black text-white shadow-lg shadow-slate-950/10 transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-500 hover:bg-emerald-500 hover:text-slate-950 hover:shadow-emerald-500/20 active:translate-y-0 active:scale-[.98] disabled:cursor-wait disabled:opacity-70 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-200">{cartBusy ? <><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> Adding…</> : cartMessage === 'Added to cart.' ? <><CheckCircle2 className="h-4 w-4" aria-hidden="true" /> Added</> : <><ShoppingCart className="h-4 w-4 transition-transform duration-200 group-hover:scale-110" aria-hidden="true" /> Add to cart</>}</button><Link href={`/order?productId=${encodeURIComponent(product.id)}&variantId=${encodeURIComponent(selected.id)}`} className="inline-flex min-h-12 items-center justify-center rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white transition duration-150 hover:bg-emerald-600 hover:text-slate-950 active:scale-[.98] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-slate-300">Order now · Cash on Delivery</Link>{cartMessage ? <p role="status" className={`mt-3 flex items-center gap-2 text-sm font-bold ${cartMessage === 'Added to cart.' ? 'text-emerald-700' : 'text-rose-700'}`}>{cartMessage === 'Added to cart.' ? <Check className="h-4 w-4 shrink-0" aria-hidden="true" /> : null}<span>{cartMessage}</span>{cartMessage === 'Added to cart.' ? <Link href="/cart" className="ml-1 inline-flex items-center gap-1 underline underline-offset-4 hover:text-emerald-900">View cart <ShoppingCart className="h-3.5 w-3.5" aria-hidden="true" /></Link> : null}</p>}</div> : <p className="mt-4 text-sm font-bold text-slate-500">This selected variant is unavailable to order.</p>}</div>
