...
export function ProductDetailInteractive({ product, settings, phone }: { product: StorefrontProduct; settings: StorefrontSettings; phone: string }) {
  ...
  const cartAdded = cartMessage === 'Added to cart.'
    ...
    <div className="grid gap-10 lg:grid-cols-[minmax(0,.92fr)_minmax(0,1.08fr)] lg:gap-16">
      ...
      <div className="min-w-0">
        ...
        <div className="mt-6 rounded-[1.5rem] border border-emerald-200 bg-emerald-50 p-5">
          ...
          <p className="mt-1 text-sm leading-6 text-slate-600">Cash on Delivery is available. Final delivery charge and stock are checked through the existing order flow.</p>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              ...
              >
                {cartBusy ? (
                  ...
                  <>
                    <ShoppingCart className="h-4 w-4 transition-transform duration-200 group-hover:scale-110" aria-hidden="true" />
                    Add to cart
                  </>
                )}
              </button>
              <Link
                ...
                className="inline-flex min-h-12 items-center justify-center rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white transition duration-150 hover:bg-emerald-600 hover:text-slate-950 active:scale-[.98] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-slate-300"
              >
                Order now · Cash on Delivery
              </Link>
              {cartMessage ? (
                <p role="status" className="mt-3 flex w-full items-center gap-2 text-sm font-bold text-slate-600">
                  {cartAdded ? <Check className="h-4 w-4 shrink-0 text-emerald-700" aria-hidden="true" /> : null}
                  <span className={cartAdded ? "text-emerald-700" : "text-rose-700"}>{cartMessage}</span>
                  {cartAdded ? (
                    <Link href="/cart" className="ml-1 inline-flex items-center gap-1 text-emerald-700 underline underline-offset-4 hover:text-emerald-900">
                      View cart <ShoppingCart className="h-3.5 w-3.5" aria-hidden="true" />
                    </Link>
                  ) : null}
                </p>
              ) : null}
