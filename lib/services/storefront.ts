...
export type StorefrontVariant = Pick<VariantRow, 'id' | 'sku' | 'variant_title' | 'ram' | 'storage' | 'color' | 'price' | 'compare_at_price' | 'stock_quantity' | 'low_stock_threshold'>
...
type RawProduct = ProductRow & { brand?: BrandRow | null; category?: CategoryRow | null; variants?: VariantRow[] | null }

const PRODUCT_SELECT = `id,brand_id,category_id,name,slug,short_description,description,product_type,status,is_featured,is_published,warranty_policy,meta_title,meta_description,created_at,updated_at,brand:brands(id,name,slug,logo_url,description,meta_title,meta_description),category:categories(id,name,slug,description,image_url,sort_order,meta_title,meta_description),variants:product_variants(id,sku,variant_title,ram,storage,color,price,compare_at_price,stock_quantity,low_stock_threshold)`

function normalizeProduct(row: RawProduct): StorefrontProduct {
...
export function getPublicAvailability(variants: StorefrontVariant[]) {
  ...
  const lowStock = variants.some((variant) => variant.stock_quantity > 0 && variant.stock_quantity <= variant.low_stock_threshold)
  if (totalStock <= 0) return { label: 'Out of stock', tone: 'out' as const }
  if (lowStock) return { label: 'Low stock', tone: 'low' as const }
...
async function resolveProductIdsForSearch(supabase: Awaited<ReturnType<typeof createClient>>, query: string) {
  ...
  const [products, brands, variants] = await Promise.all([
    ...
    supabase.from('product_variants').select('product_id').eq('is_active', true).or(`sku.ilike.%${term}%,variant_title.ilike.%${term}%,ram.ilike.%${term}%,storage.ilike.%${term}%,color.ilike.%${term}%`).limit(100),
  ])
...
async function resolveVariantIds(supabase: Awaited<ReturnType<typeof createClient>>, filters: ProductFilters) {
  ...
  let query = supabase.from('product_variants').select('product_id,price,stock_quantity,low_stock_threshold').eq('is_active', true)
  if (filters.minPrice !== undefined) query = query.gte('price', filters.minPrice)
  if (filters.maxPrice !== undefined) query = query.lte('price', filters.maxPrice)
  if (filters.availability === 'in-stock') query = query.gt('stock_quantity', 0)
  if (filters.availability === 'low-stock') query = query.gt('stock_quantity', 0)
  const { data } = await query.limit(1000)
  return (data ?? []).filter((row) => filters.availability !== 'low-stock' || row.stock_quantity <= row.low_stock_threshold).map((row) => row.product_id)
}
...
export async function getProductBySlug(slug: string) {
  ...
  return data ? normalizeProduct(data as unknown as RawProduct) : null
}
