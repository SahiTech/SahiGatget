...
function capabilitiesAnswer(locale: 'bn' | 'en') {
  return locale === 'bn'
    ? 'আমি SahiGadget-এর প্রকাশ্য তথ্যের ভিত্তিতে পণ্য খোঁজা ও তুলনা, বাজেট অনুযায়ী পণ্য, দাম ও স্টক, ভ্যারিয়েন্ট, স্পেসিফিকেশন, ওয়ারেন্টি, রিটার্ন, Cash on Delivery, অর্ডার করার নিয়ম, ডেলিভারি চার্জ/সময়সীমা এবং স্টোর/সাপোর্ট তথ্য দিতে পারি। কোনো তথ্য লাইভ ক্যাটালগ বা প্রকাশ্য নীতিতে না থাকলে আমি অনুমান না করে জানাব এবং প্রয়োজন হলে Customer Service-এর WhatsApp অপশন দেব।'
    : 'I can help with SahiGadget public information: product search and comparison, budget-based recommendations, price and stock, variants, specifications, warranty, returns, Cash on Delivery, ordering steps, delivery charges/timing, and store/support information. If something is not available in the live catalogue or public policies, I will say so instead of guessing and can hand you to Customer Service on WhatsApp when appropriate.'
}
...
function deterministicOutput(request: AssistantRequest, retrieval: RetrievalResult, intent: ReturnType<typeof classifyIntent>): AssistantModelOutput {
  ...
  if (isPrivateAssistantRequest(request.message)) {
    answer = locale === 'bn'
      ? 'আমি অর্ডার, গ্রাহক, পেমেন্ট বা অ্যাকাউন্টের ব্যক্তিগত তথ্য দেখতে পারি না। অর্ডার বা সহায়তার জন্য Customer Service-এর সাথে যোগাযোগ করুন।'
      : 'I cannot access private order, customer, payment, or account information. Please contact Customer Service for help.'
  ...
  if (intent === 'greeting') {
    answer = locale === 'bn' ? 'হ্যালো! SahiGadget-এ স্বাগতম। কীভাবে সাহায্য করতে পারি? ফোন, গ্যাজেট, বাজেট, পণ্য, অর্ডার বা ডেলিভারি সম্পর্কে জিজ্ঞেস করুন।' : 'Hello! Welcome to SahiGadget. How can I help with products, gadgets, budgets, orders, or delivery today?'
  ...
  if (intent === 'general_knowledge' || intent === 'casual_conversation') {
    answer = intent === 'casual_conversation' ? (locale === 'bn' ? 'অবশ্যই। সময় নিয়ে দেখুন—পণ্য বা SahiGadget-এর যেকোনো প্রকাশ্য তথ্য জানতে চাইলে জিজ্ঞেস করুন।' : 'Of course. Take your time—ask me about any product or public SahiGadget information when you are ready.') : generalKnowledgeFallback(request.message, locale)
    return { answer, locale, intent, productIds: [], evidenceStatus: 'partial', fallbackReason: 'none', followUps: DEFAULT_FOLLOW_UPS }
  }
  if (intent === 'support') {
    answer = locale === 'bn' ? 'আমি সাহায্য করার চেষ্টা করতে পারি। অর্ডার বাতিল, সমস্যা, ভুল তথ্য বা এমন কিছু যা আমি নিশ্চিতভাবে সমাধান করতে পারছি না—এসব ক্ষেত্রে Customer Service Team-এর WhatsApp-এ সরাসরি কথা বলাই সবচেয়ে ভালো।' : 'I can help where possible. For order cancellation, account-specific issues, incorrect information, or anything I cannot safely resolve, the best option is to speak directly with Customer Service on WhatsApp.'
  ...
  if (!first) {
    answer = locale === 'bn' ? 'দুঃখিত, আপনার চাওয়া ধরনের কোনো প্রকাশিত পণ্য বা যাচাই করা তথ্য পাইনি। আপনি চাইলে অন্যভাবে লিখুন; না হলে Customer Service-এর সাথে WhatsApp-এ সরাসরি কথা বলতে পারেন।' : 'I could not find a matching published product or verified information. Try another wording, or contact Customer Service directly on WhatsApp.'
  ...
  const variantSummary = first.variants.map((variant) => [variant.title, variant.ram, variant.storage, variant.color].filter(Boolean).join(' · ')).filter(Boolean).join(', ')
  if (locale === 'bn') {
    if (intent === 'product_comparison' && retrieval.context.length > 1) answer = `তুলনা করতে ${retrieval.context.slice(0, 3).map((item) => `${item.name} (${formatBdt(Math.min(...item.variants.map((variant) => variant.price))) ?? 'মূল্য যাচাই করুন'})`).join(' এবং ')} পাওয়া গেছে। আপনার ব্যবহারের ধরন বললে আমি কোনটি বেশি উপযোগী তা ব্যাখ্যা করতে পারি।`
    ...
    else if (intent === 'availability') answer = `${first.name} বর্তমানে ${availability === 'available' ? 'উপলভ্য' : 'স্টকে নেই'}। নির্দিষ্ট ভ্যারিয়েন্ট নির্বাচন করার পর লাইভ স্ট্যাটাস দেখুন।`
    ...
    else answer = `আপনার জন্য ${names.join(', ')} পাওয়া গেছে। লাইভ মূল্য, ছবি ও প্রাপ্যতা দেখতে পণ্যটি খুলুন।`
  } else {
    if (intent === 'product_comparison' && retrieval.context.length > 1) answer = `I found ${retrieval.context.slice(0, 3).map((item) => `${item.name} (${formatBdt(Math.min(...item.variants.map((variant) => variant.price))) ?? 'price unavailable'})`).join(' and ')}. Tell me your use case and I can explain which option is more suitable.`
...
function buildPrompt(request: AssistantRequest, retrieval: RetrievalResult, intent: ReturnType<typeof classifyIntent>, systemInstructions: string) {
  ...
  return [
    'You are SahiGadget’s production public customer-service and sales assistant.',
    'Be natural, helpful, concise, and conversational. For Bangla or Banglish, answer in natural Bangla; use English when the customer clearly prefers English.',
    'You have two knowledge layers: verified live SahiGadget context and general AI knowledge. Live store context is authoritative for products, prices, stock, variants, specifications, warranty, returns, delivery, ordering, and store policy. General knowledge may be used for generic educational questions, but never invent store-specific facts.',
    'Never invent products, prices, stock, specifications, warranty terms, delivery promises, order status, customer data, or completed actions. Never claim to access private orders or accounts.',
    'For recommendations, explain the practical trade-off when the verified context supports it. For product searches, only return productIds from the verified context.',
    'Use recent public conversation to resolve references such as “এর মধ্যে”, “এটা”, “এইটা”, “ওটা”, “this one”, or “which one”. Do not infer sensitive personal information.',
    'If the customer asks for human support, cannot be helped safely, or the request concerns an account/order-specific operation, recommend Customer Service rather than pretending to perform the action.',
    systemInstructions,
    `Detected intent: ${intent}`,
    `Customer message: ${request.message}`,
    `Recent conversation: ${conversation}`,
    `Verified context: ${context}`,
    'Return only the required JSON schema. Keep simple answers concise and useful.',
  ].join('\n\n')
}
  ...
  try {
    const response = await fetch(config.apiUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'Output JSON only. Follow the schema exactly. Ground commerce claims in verified context and never expose private data.' },
...
function isSafeModelOutput(output: AssistantModelOutput, intent: ReturnType<typeof classifyIntent>, retrieval: RetrievalResult) {
  ...
  const policyIntents = new Set(['policy', 'store_information', 'support'])
  const intentMatches = output.intent === intent || (intent === 'unclear' && (groundedIntents.has(output.intent) || policyIntents.has(output.intent)))
  ...
  if (policyIntents.has(output.intent)) return output.productIds.length === 0 && retrieval.policyText !== undefined && output.evidenceStatus === 'verified'
...
export async function buildAssistantResponse(request: AssistantRequest, requestId: string): Promise<AssistantResponse> {
  ...
  const providerEligible = config.allowRecommendations || intent === 'general_knowledge' || intent === 'casual_conversation' || intent === 'policy' || intent === 'store_information'
  const providerBlocked = ['unsupported', 'greeting', 'thanks', 'goodbye', 'clarification_required'].includes(intent)
  const providerHasUsefulContext = retrieval.context.length > 0 || Boolean(retrieval.policyText) || intent === 'general_knowledge' || intent === 'casual_conversation'
  const providerOutput = !providerBlocked && providerEligible && providerHasUsefulContext ? await callProvider(request, retrieval, intent, config) : null
