...
function consent(): Consent { try { return { necessary: true, analytics: window.localStorage.getItem(CONSENT_KEY) === 'granted', marketing: window.localStorage.getItem(CONSENT_KEY) === 'granted' } } catch { return { necessary: true, analytics: false, marketing: false } } }
function attribution() { const url = new URL(window.location.href); const keys = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'gclid', 'fbclid', 'ttclid']; const current = Object.fromEntries(keys.map((key) => [key, url.searchParams.get(key)]).filter(([, value]) => value)); const prior = JSON.parse(window.sessionStorage.getItem(ATTRIBUTION_KEY) || '{}') as Record<string, string>; const merged = { ...prior, ...current }; if (Object.keys(current).length) window.sessionStorage.setItem(ATTRIBUTION_KEY, JSON.stringify({ ...merged, landing_page: prior.landing_page || window.location.pathname })); return merged }
...
export function setAnalyticsConsent(value: 'granted' | 'denied') { window.localStorage.setItem(CONSENT_KEY, value); window.dispatchEvent(new CustomEvent('sahigadget-consent-change')) }
...
export function trackClientEvent(input: ClientEventInput) {
  ...
  if (currentConsent.analytics || input.testMode) { const gtmId = process.env.NEXT_PUBLIC_GTM_CONTAINER_ID; if (gtmId) { const w = window as typeof window & { dataLayer?: unknown[] }; w.dataLayer = w.dataLayer || []; w.dataLayer.push({ 'gtm.start': Date.now(), event: 'gtm.js' }); loadScript(`https://www.googletagmanager.com/gtm.js?id=${encodeURIComponent(gtmId)}`, 'sahigadget-gtm') } }
  if (currentConsent.analytics || input.testMode) { const idValue = process.env.NEXT_PUBLIC_GA4_MEASUREMENT_ID; if (idValue) { const w = window as typeof window & { gtag?: (...args: unknown[]) => void }; w.gtag = w.gtag || function (...args: unknown[]) { (w as typeof w & { dataLayer?: unknown[] }).dataLayer = (w as typeof w & { dataLayer?: unknown[] }).dataLayer || []; (w as typeof w & { dataLayer?: unknown[] }).dataLayer?.push(args) }; loadScript(`https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(idValue)}`, 'sahigadget-ga4'); w.gtag('js', new Date()); w.gtag('config', idValue, { send_page_view: false }); w.gtag(event.eventName, { ...event.commerce, event_id: event.eventId }) } }
  if (currentConsent.marketing || input.testMode) { const pixelId = process.env.NEXT_PUBLIC_META_PIXEL_ID; if (pixelId) { const w = window as typeof window & { fbq?: (...args: unknown[]) => void }; w.fbq = w.fbq || function (...args: unknown[]) { const q = (w as typeof w & { _fbq?: unknown[] })._fbq = (w as typeof w & { _fbq?: unknown[] })._fbq || []; q.push(args) }; loadScript('https://connect.facebook.net/en_US/fbevents.js', 'sahigadget-meta-pixel'); w.fbq('init', pixelId); w.fbq('track', event.eventName === 'view_item' ? 'ViewContent' : event.eventName === 'add_to_cart' ? 'AddToCart' : event.eventName === 'begin_checkout' ? 'InitiateCheckout' : event.eventName === 'purchase' ? 'Purchase' : event.eventName === 'search' ? 'Search' : event.eventName === 'contact' ? 'Contact' : 'PageView', { ...event.commerce, eventID: event.eventId }) } }
  if (currentConsent.analytics || input.testMode) { const w = window as typeof window & { dataLayer?: unknown[] }; w.dataLayer = w.dataLayer || []; w.dataLayer.push({ event: input.eventName, event_id: event.eventId, event_version: event.eventVersion, ecommerce: event.commerce, attribution: attribution(), test_mode: Boolean(input.testMode) }) }
  return event
}
