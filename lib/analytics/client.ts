...
export function trackClientEvent(input: ClientEventInput) {
  ...
  if (currentConsent.marketing || input.testMode) { const pixelId = process.env.NEXT_PUBLIC_META_PIXEL_ID; if (pixelId) { const w = window as typeof window & { fbq?: (...args: unknown[]) => void }; w.fbq = w.fbq || function (...args: unknown[]) { const q = (w as typeof w & { _fbq?: unknown[] })._fbq = (w as typeof w & { _fbq?: unknown[] })._fbq || []; q.push(args) }; loadScript('https://connect.facebook.net/en_US/fbevents.js', 'sahigadget-meta-pixel'); w.fbq('init', pixelId); w.fbq('track', input.eventName === 'view_item' ? 'ViewContent' : input.eventName === 'add_to_cart' ? 'AddToCart' : input.eventName === 'begin_checkout' ? 'InitiateCheckout' : input.eventName === 'purchase' ? 'Purchase' : input.eventName === 'search' ? 'Search' : input.eventName === 'contact' ? 'Contact' : 'PageView', { ...event.commerce, eventID: event.eventId }) } }
  return event
}
