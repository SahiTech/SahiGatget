...
export async function createCartCodOrder(input: unknown) {
  ...
  const risk = await assessCustomerRisk({ phone: parsed.data.phone })
  ...
  await markCheckoutSession({ checkoutRequestId: parsed.data.checkoutRequestId, source: 'CART', cartId: cart.id, status: 'COMPLETED', customerPhone: parsed.data.phone, customerEmail: parsed.data.email || null, completedOrderId: row.order_id })
  ...
  return { ok: true, data: { orderId: row.order_id, orderNumber: row.order_number, createdNew: row.created_new } }
}
